...
class App extends Component {
  ...
  componentDidMount = () => {
    this.intervalId = setInterval(this.timerChange, 1000)
  }
  ...
  onClickAnyThumbnail = id => {
    ...
    console.log(timer > 0 && imagesList[randomNumber].id === id)
    if (timer > 0 && imagesList[randomNumber].id === id) {
  ...
  onClickPlayAgain = () => {
    this.setState({isResult: false, score: 0, timer: 60})
  }
  ...
  render() {
    const {activeTabId, randomNumber, score, timer, isResult} = this.state
        ...
        <nav className="nav-container">
          ...
          <ul className="container">
            <li className="list container">
              <p className="list">Score: </p>
              <p className="score">{score}</p>
            </li>
            <li className="list container">
              <img
                className="logo-timer"
              ...
              <div className="bottom-container">
                ...
                <ul className="image-container">
                  {filterList.map(eachItem => (
                    <ImageItem
                      key={eachItem.id}
                      onClickAnyThumbnail={this.onClickAnyThumbnail}
                      imageDetail={eachItem}
                    />
                  ))}
                </ul>
              </div>
